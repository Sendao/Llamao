...
Napi::Function NodeModelWrapper::GetClass(Napi::Env env)
{
    Napi::Function self = DefineClass(env, "LLModel",
                                      {InstanceMethod("type", &NodeModelWrapper::GetType),
                                       ...
                                       InstanceMethod("selectContext", &NodeModelWrapper::SelectContext),
                                       InstanceMethod("viewContext", &NodeModelWrapper::ViewContext),
                                       InstanceMethod("tokenLookup", &NodeModelWrapper::TokenLookup)
                                   } );
...
Napi::Value NodeModelWrapper::TokenLookup(const Napi::CallbackInfo &info)
{
    int x = info[0].As<Napi::Number>().Int32Value();
    return Napi::String::New(info.Env(), llmodel_token_lookup(GetInference(), x));
}
...
Napi::Value NodeModelWrapper::SaveState(const Napi::CallbackInfo &info)
{
    auto i = GetInference();
    auto e = info.Env();
    //auto sz = llmodel_get_state_size(i);
    //uint8_t *data = new uint8_t[sz];
    auto buf = Napi::Buffer<uint8_t>::Buffer(e, info[0]);
    uint8_t *data = (uint8_t*)buf.Data();
    return Napi::Number::New(e, static_cast<int64_t>(llmodel_save_state_data(i, data)));
}
Napi::Value NodeModelWrapper::RestoreState(const Napi::CallbackInfo &info)
{
    auto i = GetInference();
    auto e = info.Env();
    //auto sz = llmodel_get_state_size(i);
    ...
    auto buf = Napi::Buffer<uint8_t>::Buffer(e, info[0]);
    uint8_t *data = (uint8_t*)buf.Data();
    return Napi::Number::New(info.Env(), static_cast<int64_t>(llmodel_restore_state_data(i, const_cast<uint8_t*>(data) )));
}
Napi::Value NodeModelWrapper::SelectContext(const Napi::CallbackInfo &info)
{
    auto i = GetInference();
    auto e = info.Env();
    return Napi::Boolean::New(e, static_cast<bool>(llmodel_select_context(i, (uint8_t)info[0].As<Napi::Number>().Uint32Value())));
}
Napi::Value NodeModelWrapper::ViewContext(const Napi::CallbackInfo &info)
{
    auto i = GetInference();
    auto e = info.Env();
    return Napi::Number::New(e, static_cast<uint8_t>(llmodel_view_context(i, info[0].As<Napi::Number>().Uint32Value())));
}
...
Napi::Value NodeModelWrapper::Infer(const Napi::CallbackInfo &info)
{
    ...
    if (inputObject.Has("continuing"))
    {
        promptContext.continuing = inputObject.Get("continuing").As<Napi::Boolean>().Value();
        //std::cerr << "Continuing = " << promptContext.continuing << "\n";
    }
    if (inputObject.Has("per_idle") && inputObject.Get("per_idle").IsNumber())
    {
        promptContext.per_idle = inputObject.Get("per_idle").As<Napi::Number>().Int32Value();
    }
    ...
    if( prompt.length() == 0 )
        std::cerr << "Empty prompt:" << (promptContext.continuing?"cont":"non") << "\n";
