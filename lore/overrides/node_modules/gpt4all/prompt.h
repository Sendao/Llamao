...
class PromptWorker : public Napi::AsyncWorker
{
  public:
    ...
    bool ResponseCallback(int32_t token_id, const std::string token, int lsz, int esz, float *logits, float *embds);
    bool RecalculateCallback(bool isrecalculating);
    bool PromptCallback(int32_t token_id, int lsz, int esz, float *logits, float *embds);
