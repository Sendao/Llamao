...
char *llmodel_token_lookup(llmodel_model model, int token)
{
    auto *wrapper = static_cast<LLModelWrapper *>(model);
    std::string res = wrapper->llModel->tokenLookup(token);
...
bool llmodel_select_context(llmodel_model model, uint8_t ctx_n)
{
    auto *wrapper = static_cast<LLModelWrapper *>(model);
    return wrapper->llModel->selectContext(ctx_n);
}
uint8_t llmodel_view_context(llmodel_model model, uint8_t ctx_n)
{
    auto *wrapper = static_cast<LLModelWrapper *>(model);
    return wrapper->llModel->viewContext(ctx_n);
}
void llmodel_release_context(llmodel_model model, uint8_t ctx_n)
{
    auto *wrapper = static_cast<LLModelWrapper *>(model);
    return wrapper->llModel->releaseContext(ctx_n);
}
                    ...
                    const char *fake_reply)
{
    ...
    auto response_func = [response_callback](int32_t token_id, const std::string &response, int lsz, int esz, float *logits, float *embds) {
        return response_callback(token_id, response.c_str(), lsz, esz, logits, embds);
    };
    ...
    wrapper->llModel->prompt(prompt, prompt_template, prompt_callback, response_func,
